...
def race_step(car: Car, racetrack: Racetrack) -> None:
    if car.current_speed < car.top_speed:
        ...
        car.current_speed += car.acceleration_start * (
            (1 / (1 + car.current_speed / car.top_speed)) - 0.5
        )
    ...
    log.debug(
        ...
        extra={
            ...
            "completion": car.track_completion,
...
def display_results(*args) -> None:
    if len(args) == 2:
        ...
        log.info(
            "winner", extra={"winner": f"{args[0].year} {args[0].make} {args[0].model}"}
        )
